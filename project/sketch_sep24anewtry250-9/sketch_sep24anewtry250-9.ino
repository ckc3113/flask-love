...
// WiFi credentials
const char* ssid = "fastconsb 5G@unifi";
...
// Limit Switch Pins
#define X_LIMIT_PIN 18  // Pin for X axis limit switch
#define Y_LIMIT_PIN 19  // Pin for Y axis limit switch
#define Z_LIMIT_PIN 20  // Pin for Z axis limit switch
...
long positions[5][3] = {
  ...
  {3000, 2000, 1000},   // Position 2
  {1000, 4000, 1000},   // Position 3 
  {3000, 6000, 1000},   // Position 4
...
void setup() {
  ...
  // Set limit switch pins as input with pullup resistors
...
void loop() {
  ...
  // Check if limit switches are triggered and stop motors
  if (digitalRead(X_LIMIT_PIN) == LOW) {
    stepperX.stop();  // Stop X motor if limit switch triggered
    stepperX.setCurrentPosition(0);  // Set current position as home (0)
    Serial.println("X limit switch triggered, X axis homed.");
  }
  if (digitalRead(Y_LIMIT_PIN) == LOW) {
    stepperY.stop();  // Stop Y motor if limit switch triggered
    stepperY.setCurrentPosition(0);  // Set current position as home (0)
    Serial.println("Y limit switch triggered, Y axis homed.");
  }
  if (digitalRead(Z_LIMIT_PIN) == LOW) {
    stepperZ.stop();  // Stop Z motor if limit switch triggered
    stepperZ.setCurrentPosition(0);  // Set current position as home (0)
    Serial.println("Z limit switch triggered, Z axis homed.");
  }
...
void moveToPosition(int compartmentIndex) {
  ...
  while (stepperX.distanceToGo() != 0 || stepperY.distanceToGo() != 0 || stepperZ.distanceToGo() != 0) {
    // Run motors, but stop if limit switch is triggered
    if (digitalRead(X_LIMIT_PIN) == LOW) {
      stepperX.stop();
      stepperX.setCurrentPosition(0);  // Set X axis home
    } else {
      stepperX.run();
    }
    ...
    if (digitalRead(Y_LIMIT_PIN) == LOW) {
      stepperY.stop();
      stepperY.setCurrentPosition(0);  // Set Y axis home
    } else {
      stepperY.run();
    }
    ...
    if (digitalRead(Z_LIMIT_PIN) == LOW) {
      stepperZ.stop();
      stepperZ.setCurrentPosition(0);  // Set Z axis home
    } else {
      stepperZ.run();
    }
  }

  // Return to home position after moving to the compartment
  returnToHome();
}
...
void sendToGoogleSheets(String barcode) {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    Serial.println("WiFi not connected. Unable to send data to Google Sheets.");
  }
}
