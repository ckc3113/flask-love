...
// Define limit switch pins (X, Y, and Z axes, NC type)
...
void setup() {
  ...
  // Limit switch setup (NC type)
  pinMode(X_LIMIT_SWITCH_PIN, INPUT_PULLUP);  // Use INPUT_PULLUP for NC switch (X axis)
  pinMode(Y_LIMIT_SWITCH_PIN, INPUT_PULLUP);  // Use INPUT_PULLUP for NC switch (Y axis)
  pinMode(Z_LIMIT_SWITCH_PIN, INPUT_PULLUP);  // Use INPUT_PULLUP for NC switch (Z axis)
...
void moveAxesTogether(int steps) {
  ...
  while (!stopX || !stopY || !stopZ) {
    if (!stopX) {
      ...
      // Check for NC switch: now we check for HIGH when the limit switch is triggered
    ...
    if (!stopY) {
      ...
      if (digitalRead(Y_LIMIT_SWITCH_PIN) == HIGH) {
    ...
    if (!stopZ) {
      ...
      if (digitalRead(Z_LIMIT_SWITCH_PIN) == HIGH) {
